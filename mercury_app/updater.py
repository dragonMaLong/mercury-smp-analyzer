...
def _download_dir() -> Path:
    if os.name == "nt":
        ...
        if base:
            return Path(base) / "MercurySMPAnalyzerZh" / "updates"
    return Path.home() / ".mercury_smp_analyzer_zh" / "updates"


def _download_filename(info: UpdateInfo) -> str:
    raw_name = (info.asset_name or "").strip() or "Mercury-SMP-Analyzer-zh-CN.exe"
    suffix = Path(raw_name).suffix or ".exe"
    version = re.sub(r"[^0-9A-Za-z._-]+", "_", info.latest_version or "update")
    return f"Mercury-SMP-Analyzer-zh-CN-v{version}{suffix}"
...
try {{
    ...
    Write-UpdateLog "wait old process failed: $($_.Exception.Message)"
}}
...
try {{
    ...
    try {{
        ...
        Write-UpdateLog "remove old backup failed: $($_.Exception.Message)"
    }}
    ...
    Write-UpdateLog "replacement complete"
    Show-UpdateMessage "更新成功！立即重启软件" "软件更新" "Information"
    Start-UpdatedExecutable $OldPath $TargetDir
}} catch {{
    $message = $_.Exception.Message
    Write-UpdateLog ("apply update failed: " + $message)
    try {{
        if (!(Test-Path -LiteralPath $OldPath) -and (Test-Path -LiteralPath $BackupPath)) {{
            Move-Item -LiteralPath $BackupPath -Destination $OldPath -Force
            Write-UpdateLog "restored old executable from backup"
        }}
    }} catch {{
    ...
    try {{
        ...
        Write-UpdateLog "open update folder failed: $($_.Exception.Message)"
    }}
}}
"""
