...
DEFAULT_UPDATE_REPOSITORY = "dragonMaLong/mercury-smp-analyzer"
DEFAULT_GITEE_MANIFEST_URL = (
    "https://gitee.com/dragonMalong/mercury-smp-analyzer/raw/main/updates/latest.json"
)
DEFAULT_GITHUB_MANIFEST_URL = (
    "https://raw.githubusercontent.com/dragonMaLong/mercury-smp-analyzer/main/updates/latest.json"
)
...
PREFERRED_ASSET_KEYWORDS = ("zh-cn", "zh_cn", "zh", "cn", "中文", "压汞")
...
def _info_from_manifest(payload: dict[str, Any], current_version: str, source_url: str) -> UpdateInfo:
    ...
    return UpdateInfo(
        ...
        source_name=str(
            payload.get(f"{source_prefix}_source_name")
            or detected_source
...
def _fetch_json(url: str, *, timeout: float, api: bool) -> dict[str, Any]:
    headers = {
        ...
        "User-Agent": f"Mercury-SMP-Analyzer/{__version__}",
    }
    if api:
...
def _choose_download_asset(assets: list[Any]) -> dict[str, Any] | None:
    ...
    for suffix in preferred_suffixes:
        for asset in candidates:
            ...
            if name.endswith(suffix) and _is_preferred_asset(name) and asset.get("browser_download_url"):
                return asset
...
def _is_preferred_asset(name: str) -> bool:
    normalized = name.replace("-", "_")
    return any(keyword in normalized for keyword in PREFERRED_ASSET_KEYWORDS)
